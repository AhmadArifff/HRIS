      ...
      <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative animate-modal-book-open text-white">
        ...
        <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-800">
          ...
          <button
            ...
            disabled={isScanning || faceVerified}
            className="px-6 py-3 text-xs font-bold text-white bg-gradient-to-r from-brand-600 to-indigo-600 hover:from-brand-500 hover:to-indigo-500 rounded-xl transition-all shadow-lg shadow-brand-500/25 flex items-center gap-2.5 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
            {isScanning ? (
              <>
                <div className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin"></div>
                <span>Memverifikasi Biometrik Wajah...</span>
              </>
            ) : (
              <>
                ...
                <span>Pindai Wajah & Masuk Portal (Sesi 15 Min)</span>
              </>
            )}
          </button>
        </div>
